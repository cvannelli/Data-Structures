...
* Date: 03-09-16
* Description: function that create the 'graph'
*  data structure
*/
...
int DFS(Graph* g, Vertex* source, Vertex* destination)
{
	...
	int path = 0;
	/*boolean indicating whether path exists or not*/

	cirListDeque *stack = malloc(sizeof(cirListDeque));
	/*creates a stack*/

	initCirListDeque(stack);
	/*initializes size and last to 0*/

	addFrontCirListDeque(stack, source);
	/*adds the source to the top of the stack*/

	while (!isEmptyCirListDeque(stack))
	{
		Vertex *currVert = frontCirListDeque(stack);
		...
		for (int i = 0; i < currVert->numNeighbors; ++i)
		{
			if (!(currVert->neighbors[i]->isVisited))
			{
				addFrontCirListDeque(stack, currVert->neighbors[i]);
				/*adds neighbors to the top of the stack*/
			}
		}
...
int BFS(Graph* g, Vertex* source, Vertex* destination)
{
	...
	/*adds the source to the front of the queue*/

	while (!isEmptyCirListDeque(queue))
	{
		Vertex *currVert = frontCirListDeque(queue);
		/*sets the current vertex to the front of the queue*/
		...
		for (int i = 0; i < currVert->numNeighbors; ++i)
		{
			if (!(currVert->neighbors[i]->isVisited))
			{
				...
				/*adds neighbors to back of the queue*/
			}
		}
