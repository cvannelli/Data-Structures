...
**Date: 02/25/16
**Description: contains dynamic array implementation
** as well as heap functions.
*/
...
int _smallerIndexHeap(DynArr *heap, int i, int j, comparator compare)
{
	...
	/*confirms indexes i and j are smaller than the heap size*/
	...
	{
		return i;
		/* returns if left is smaller than right*/
	}
...
void addHeap(DynArr *heap, TYPE val, comparator  compare)
{
	...
	/*adds to heap while position is not 0*/
	while (pos != 0)
	{
		parentNode = (pos - 1) / 2;
		if (compare(getDynArr(heap, pos), getDynArr(heap, parentNode)) == -1)
		{
			swapDynArr(heap, parentNode, pos);
			pos = parentNode;
			/*swaps pos with parent node if pos is smaller than parentNode*/
		}
		else
			return;
	}
...
void _adjustHeap(DynArr *heap, int max, int pos, comparator compare)
{
	...
	{ /* we have two children */
		int minIndex = _smallerIndexHeap(heap, leftChild, rightChild, compare);

		if (compare(getDynArr(heap, pos), getDynArr(heap  ,minIndex)) == 1) /*value at pos > value of smallest child*/
		{
			swapDynArr(heap, pos, minIndex);
			/*swap with smallest child */

			_adjustHeap(heap, max, minIndex, compare);
		}
	}
	else if (leftChild < max)
	{ /* we have one child */
		if (compare(getDynArr(heap, pos), getDynArr(heap, leftChild)) == 1) /*value at pos > value of child */
		{
...
void _buildHeap(DynArr *heap, comparator compare)
{
	...
	int firstIndex = (sizeDynArr(heap) / 2) - 1;
	/* first index is half the size of heap minus 1*/

	for (int i = firstIndex; i >= 0; i--)
	{
		_adjustHeap(heap, sizeDynArr(heap) - 1, i, compare);
		/*adjusts heap continuously down to 0 index*/
	}
}
/*
...
void sortHeap(DynArr *heap, comparator compare)
{
	...
	/* obtains the last index*/

	while (lastIndex > 0)
	{
		swapDynArr(heap, 0, lastIndex);
		_adjustHeap(heap, lastIndex, 0, compare);
		lastIndex--;

		/*while last index is greater than 0, swap the first and last
		** index and continuously adjust the heap while decrementing
		** the lastIndex
		*/
	}
}
