...
** Date: 02/11/16
** Description: contains functions and tests for the
** binary search tree
*/
...
struct Node *_addNode(struct Node *cur, TYPE val)
{
	...
	if (compare(val, cur->val) < 0)
	{
		...
		/* set the left child to be the value returned by add(leftChild, val)*/
	}
...
int containsBSTree(struct BSTree *tree, TYPE val)
{
	...
	struct Node * tempNode = tree->root;
	/*pointer to a node created*/

	while (tempNode != 0)
	{
		if (compare(val, tempNode->val) == 0)
		{
			return 1;
		}

		else if (compare(val, tempNode->val) == -1)
		{
...
{
	assert(cur != 0);
	/*confirms cur is not null*/

	while (cur->left != 0)
	{
		cur = cur->left;
		/*iterates toward the left-most node*/
	}
...
struct Node *_removeNode(struct Node *cur, TYPE val)
{
	...
	else if (compare(val, cur->val) == -1)
	{
		...
		/*set left child to remove(left child, val)*/
	}
	else
	{
		cur->right = _removeNode(cur->right, val);
		/* set right child to remove (right child, val)*/
	}
